...
<main id="main">
  <section id="overview" class="paper"><p class="kicker">01 / Project map</p><div class="section-grid"><div><h2>The complete project in one page.</h2><p>The question is whether Wasserstein-robust Q-learning preserves one-year-ahead adjusted antibiotic coverage when national resistance patterns drift. The answer from this formulation is no: robustness raised raw susceptibility but selected broader treatment and reduced the prespecified endpoint.</p></div><div class="plain-grid"><div class="plain-card"><b>Decision maker</b>Population-informed policy selecting one antibiotic class for a country-year resistance state.</div><div class="plain-card"><b>Comparison</b>Classical tabular Q-learning versus Wasserstein-robust tabular Q-learning.</div><div class="plain-card"><b>Evaluation</b>2020-2023 decisions scored against observed 2021-2024 outcomes.</div><div class="plain-card"><b>Primary endpoint</b>Mean next-year susceptibility minus 0.10 for carbapenem use.</div><div class="plain-card"><b>Main finding</b>Robust adjusted coverage was 0.36 percentage points lower at epsilon-star.</div><div class="plain-card"><b>Scope</b>Country-level methodological study, not patient-level prescribing guidance.</div></div></div></section>

  <section id="data"><p class="kicker">02 / Data and cohort</p><div class="section-grid"><div><h2>What was observed.</h2><p>The source is public EARS-Net surveillance for invasive <em>Escherichia coli</em> isolates. Three antibiotic-class exports provide resistance percentages and tested-isolate counts from 2015 through 2024.</p><p>Countries required at least three complete training transitions and two complete evaluation transitions. Missing values were not imputed. Of {initial_country_count} observed countries, {country_count} met the rule, producing {country_year_rows} retained country-year rows and {transition_count} held-out transitions.</p></div><div><div class="note"><strong>Why this matters:</strong> the data describe national surveillance populations. They do not contain diagnoses, doses, contraindications, clinical severity, or individual treatment outcomes.</div><h3>How to read the figures</h3><p>The resistance-trend figure averages countries equally. It shows that fluoroquinolone and third-generation cephalosporin resistance were much higher than carbapenem resistance. The coverage bars show where complete observations support training and evaluation; they are not patient counts.</p></div></div><div class="figure-grid"><figure><div class="figure-art">{figures["resistance-trends.svg"]}</div><figcaption><strong>Figure 1.</strong> Equal-country mean resistance. This is descriptive context, not an estimate of treatment effectiveness.</figcaption></figure><figure><div class="figure-art">{figures["data-coverage.svg"]}</div><figcaption><strong>Figure 2.</strong> Complete country-year records. Navy years train the model; rust years support held-out evaluation.</figcaption></figure></div></section>

  <section id="model" class="paper"><p class="kicker">03 / State, action, transition, reward</p><div class="section-grid"><div><h2>The eight-state MDP.</h2><p>Each state contains three binary indicators in this order: third-generation cephalosporin resistance, fluoroquinolone resistance, and carbapenem resistance. The first two thresholds are training-only medians ({float(thresholds.get("resistance_3gc_median", 0)):.2f}% and {float(thresholds.get("resistance_fq_median", 0)):.2f}%); carbapenem is encoded as zero versus greater than zero.</p><div class="equation">Primary reward r(x,a,x') = susceptibility(x',a) - 0.10 x I(a = carbapenem)</div><p>Actions are `0` third-generation cephalosporin, `1` fluoroquinolone, and `2` carbapenem. A policy such as <code>20222222</code> lists one action for each state from 0 to 7.</p></div><div><table class="compact"><thead><tr><th>State</th><th>Bits</th><th>Interpretation</th></tr></thead><tbody>{state_rows}</tbody></table></div></div><div class="note"><strong>Critical modeling boundary:</strong> the transition kernel is action-independent. Choosing an antibiotic does not change the modeled next resistance state. Therefore, future-resistance effects are not estimated causally, and the environment behaves close to a state-wise decision problem.</div></section>

  <section id="algorithm"><p class="kicker">04 / Learning algorithm</p><div class="section-grid"><div><h2>Classical versus Wasserstein-robust Q-learning.</h2><p>Classical Q-learning estimates the long-run value of each state-action pair from sampled state trajectories. An epsilon-greedy behavior rule explores alternative actions, and the learning rate decreases as each pair is visited.</p><p>The robust learner replaces the nominal expected continuation value with the worst value inside a Wasserstein ball around the estimated transition distribution. Normalized Hamming distance measures how many of the three resistance bits differ between states.</p></div><div><div class="plain-card"><b>Ambiguity center</b>Pooled training transition kernel with total smoothing gamma = 0.10.</div><div class="plain-card"><b>Ground cost</b>Normalized Hamming distance on the eight binary states; Wasserstein order q = 1.</div><div class="plain-card"><b>Radius</b>Median historical annual W1 drift, epsilon-star = {epsilon_star:.4f}; evaluated at 0.5x, 1x, 1.5x, and 2x.</div><div class="plain-card"><b>Verification</b>Robust dual calculations were checked against finite linear-program primal examples, and learned policies were compared with exact Bellman solutions.</div></div></div><div class="figure-grid"><figure><div class="figure-art">{figures["wasserstein-drift.svg"]}</div><figcaption><strong>Figure 3.</strong> Historical drift calibrates uncertainty independently of whether the resulting policy looks favorable.</figcaption></figure><figure><div class="figure-art">{figures["policy-by-state.svg"]}</div><figcaption><strong>Figure 4.</strong> The exact classical policy varies in state 1; the primary robust policy selects carbapenems in all states.</figcaption></figure></div></section>

  <section id="training" class="paper"><p class="kicker">05 / Training and validation</p><div class="section-grid"><div><h2>How parameters were selected.</h2><p>Two rolling-origin folds trained through 2017 and 2018, then validated on 2017-to-2018 and 2018-to-2019 transitions. Discount factors 0.3, 0.45, 0.7, and 0.9 and exploration rates 0.05, 0.1, and 0.2 were tuned separately for each algorithm over ten fixed seeds.</p><p>The selected classical configuration was discount {float(classical_config.get("discount", 0)):.2f}, exploration {float(classical_config.get("exploration_rate", 0)):.2f}; the robust configuration was discount {float(robust_config.get("discount", 0)):.2f}, exploration {float(robust_config.get("exploration_rate", 0)):.2f}.</p></div><div><div class="metric-grid"><div class="metric"><strong>50,000</strong><span>updates per final learner</span></div><div class="metric"><strong>10</strong><span>fixed tuning seeds</span></div><div class="metric"><strong>30</strong><span>fixed final seeds</span></div></div><p>Ties were broken by lower seed variability, then proximity to the paper defaults (0.45, 0.10). Exact Bellman policies provide deterministic endpoint values; seeded learners quantify finite-run stability.</p></div></div></section>
  ...
  <section id="results" class="result-band"><p class="kicker">07 / Primary result</p><blockquote>Robustness increased susceptibility, but not the endpoint that penalized broad use.</blockquote><p>The classical exact policy <code>20222222</code> achieved 90.13% adjusted coverage, 96.58% raw susceptibility, and 64.55% carbapenem use. At epsilon-star, the robust exact policy <code>22222222</code> achieved 89.76% adjusted coverage, 99.76% raw susceptibility, and 100% carbapenem use.</p><p>Relative to classical Q-learning, the robust policy changed adjusted coverage by {adjusted_difference:+.2f} percentage points, raw susceptibility by +{susceptibility_difference:.2f} points, and carbapenem use by +{carb_difference:.2f} points. It also had greater regret to the hindsight oracle. The primary hypothesis was therefore not supported.</p><div class="figure-grid"><figure><div class="figure-art">{figures["performance-by-year.svg"]}</div><figcaption><strong>Figure 5.</strong> Classical adjusted coverage exceeded the robust policy in every held-out outcome year.</figcaption></figure><figure><div class="figure-art">{figures["coverage-tradeoff.svg"]}</div><figcaption><strong>Figure 6.</strong> Higher susceptibility was obtained by moving to universal carbapenem selection, not by improving the adjusted endpoint.</figcaption></figure></div></section>

  <section id="baselines" class="paper"><p class="kicker">08 / Baselines and sensitivities</p><div class="section-grid"><div><h2>What the policies were compared against.</h2><table class="compact"><thead><tr><th>Baseline</th><th>Adjusted coverage</th><th>Raw susceptibility</th><th>Carbapenem use</th></tr></thead><tbody>{baseline_rows}</tbody></table><p>The classical exact policy equals the myopic state-wise policy in this action-independent environment. That agreement is expected and is an important interpretation, not an implementation failure.</p></div><div><h3>Prespecified sensitivity findings</h3><ul><li>Tested-isolate weighting selected carbapenems in every state for both algorithms.</li><li>Adding breadth and future-pressure penalties reduced carbapenem use for some classical policies, but robust policies remained broader.</li><li>The cost-adjusted classical policy <code>00022222</code> reduced carbapenem use to 49.09%; the robust epsilon-star policy increased it to 64.55%.</li><li>Fluoroquinolones were not selected by any reported exact policy, indicating dominance under the estimated rewards rather than a requirement that every action appear.</li></ul><div class="note">Rewards from different definitions are not numerically comparable. Their purpose is to show how normative objectives change policy composition.</div></div></div></section>

  <section id="convergence"><p class="kicker">09 / Convergence and uncertainty</p><div class="section-grid"><div><h2>What “not converged” means here.</h2><p>The strict criterion required every one of the 30 seeded greedy policies to match the exact Bellman policy. No final training group met that criterion. Primary mean policy agreement was 90.00% for classical Q-learning and approximately 96.67% at epsilon-star for robust Q-learning.</p><p>This does not make the exact finite-state comparison invalid; it means the sampled Q-learning implementation retained finite-run variation, particularly in states with small action-value margins.</p></div><div><div class="plain-card"><b>Reported together</b>Exact policy, modal seeded policy, state agreement, Bellman sup-norm error, visits, and seed-level outcomes.</div><div class="plain-card"><b>Not claimed</b>Asymptotic convergence, statistical significance, confidence intervals, causal effects, or clinical safety.</div></div></div></section>
