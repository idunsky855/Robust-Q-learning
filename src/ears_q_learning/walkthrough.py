...
<main id="main">
  <section id="question" class="paper"><div class="section-grid"><div><p class="kicker">01 / Research question</p><h2>Does robustness preserve useful coverage under drift?</h2></div><div><p>The project compares classical and Wasserstein-robust tabular Q-learning for population-informed empiric antibiotic selection. It uses invasive <em>Escherichia coli</em> surveillance, not patient records.</p><div class="metric-grid"><div class="metric"><strong>{country_count}</strong><span>eligible countries</span></div><div class="metric"><strong>{transition_count}</strong><span>held-out transitions</span></div><div class="metric"><strong>8 x 3</strong><span>states and actions</span></div></div></div></div><div class="figure-grid"><figure><div class="figure-art">{figures["resistance-trends.svg"]}</div><figcaption>Country-level resistance means provide the empirical context.</figcaption></figure><figure><div class="figure-art">{figures["data-coverage.svg"]}</div><figcaption>No missing resistance values were imputed.</figcaption></figure></div></section>
  <section id="method"><p class="kicker">02 / Decision model</p><div class="section-grid"><div><h2>A small MDP with an explicit boundary.</h2><p>Three binary resistance indicators define each state. Actions select an antibiotic class. Resistance transitions are action-independent, so the model does not claim that recommendations change future resistance.</p></div><div><div class="equation">Primary reward = next-year susceptibility - 0.10 x I(carbapenem)</div><div class="decision-trail"><div class="decision"><h3>Temporal separation</h3><p>Transitions ending by 2019 train the model; decisions from 2020-2023 are evaluated on 2021-2024 outcomes.</p></div><div class="decision"><h3>Radius calibration</h3><p>The Wasserstein radius is the median historical annual W1 distance: {epsilon_star:.4f}.</p></div><div class="decision"><h3>Independent tuning</h3><p>Classical and robust learners use rolling-origin validation, followed by 30 fixed seeds and exact Bellman checks.</p></div></div></div></div><div class="figure-grid"><figure><div class="figure-art">{figures["wasserstein-drift.svg"]}</div><figcaption>The ambiguity radius is calibrated independently of policy performance.</figcaption></figure><figure><div class="figure-art">{figures["policy-by-state.svg"]}</div><figcaption>The primary robust policy collapses to carbapenems in all eight states.</figcaption></figure></div></section>
  <section id="results" class="result-band"><p class="kicker">03 / Result</p><blockquote>Robustness increased susceptibility, but not the endpoint that penalized broad use.</blockquote><p>The classical exact policy achieved 90.13% adjusted coverage with 64.55% carbapenem use. At epsilon-star, the robust exact policy achieved 89.76% adjusted coverage with 100% carbapenem use. No primary training group met the strict all-seed convergence criterion.</p><div class="figure-grid"><figure><div class="figure-art">{figures["performance-by-year.svg"]}</div><figcaption>Classical adjusted coverage remained above the robust policy in each outcome year.</figcaption></figure><figure><div class="figure-art">{figures["coverage-tradeoff.svg"]}</div><figcaption>Higher raw susceptibility coincided with universal carbapenem selection.</figcaption></figure></div></section>
